...
local directions = Difficulty.getConfig("easy").directions
local menu = nil
local player = nil
...
local function startRound()
    currentDifficulty = menu.selectedDifficulty
    directions = Difficulty.getConfig(currentDifficulty).directions
    resetGame()
...
local function endRound()
    isRoundActive = false
    targets = {}
    menu:addScore(score, menu.currentPlayerName)
end
...
function love.update(dt)
    ...
    if spawnTimer <= 0 then
        ...
        local newTarget = Target.spawnRandom(WIDTH, HEIGHT, direction)
    ...
    for i = #targets, 1, -1 do
        ...
        target.elapsedTime = target.elapsedTime + dt
        target.lifeTime = target.lifeTime - dt
        if target.lifeTime <= 0 then
...
function love.draw()
    ...
    local difficultyConfig = Difficulty.getConfig(currentDifficulty)
    local controlsText = "Controls: "
    if currentDifficulty == "easy" then
        controlsText = controlsText .. "Arrow Keys or WASD"
    elseif currentDifficulty == "medium" then
        controlsText = controlsText .. "ASDFGHJKL"
    elseif currentDifficulty == "hard" then
        controlsText = controlsText .. "QWERTYUIOPASDFGHJKLZXCVBNM"
    elseif currentDifficulty == "extreme" then
        controlsText = controlsText .. "ABCDEFGHIJKLMNOPQRSTUVWXYZ0-9"
    end
    love.graphics.print(controlsText, 10, 58)

    if currentState == States.game.paused then
        love.graphics.printf("Paused. Press R to resume or Escape to return to menu.", 0, HEIGHT / 2 - 20, WIDTH, "center")
    ...
    if currentState == States.game.menu then
        ...
        if selection == "Play" then
            if menu.currentPlayerName == "" then
                menu:resetNameEntry()
                menu.state = States.menu.nameInput
            else
                menu.state = States.menu.difficultySelect
            end
...
function love.keypressed(key)
    ...
    if isRoundActive then
        ...
        if direction then
            for i = #targets, 1, -1 do
                local target = targets[i]
                if target and target.isAlive and target.directionType == direction then
                    score = score + target:getPointValue()
                    target.isAlive = false
                    table.remove(targets, i)
                    break
                end
...
function love.textinput(text)
    if currentState == States.game.menu and menu.state == States.menu.nameInput then
        menu:textinput(text)
    end
end
